...
import { PresupuestoViewComponent } from '../views/presupuesto/view';
...
export type DashSectionType = 'bar' | 'histogram' | 'donut' | 'completeness' | 'table' | 'cards' | 'text_list';
export interface DashSection {
  id: string; title: string; subtitle: string; type: DashSectionType;
  bars?: DashBar[]; segments?: DashSegment[];
  columns?: string[]; rows?: any[]; cards?: any[]; texts?: string[];
}
...
export interface DashboardData {
  ...
  dataset_type: 'personas_capacitadas' | 'animales' | 'presupuesto' | 'generico'; // agregado presupuesto
}
